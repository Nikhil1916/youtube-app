...
const Head = () => {
  ...
  useEffect(()=>{  
    ...
    const getSuggestions = async() => {
      if(searchCache.has(searchInput)) {
        setSuggestions(searchCache.get(searchInput));
        return;
      }
   ...
   <div className='grid grid-flow-col px-5 py-2 mb-2 shadow-lg bg-white' id="header">
        ...
        <div className='col-span-10'>
          <div className="flex items-center">
              <input type='text' className='h-10 p-2 rounded w-[80%] border border-gray-200 rounded-l-full focus:border-yellow-600' placeholder='Search' value={searchInput} onChange={(e)=>{handleSearch(e)}} onFocus={()=>setShowSuggestions(true)} onBlur={()=>setShowSuggestions(false)} />
          ...
          </div>
        {
          showSuggestion  && suggestion?.length > 0 &&  <div className="fixed bg-white py-2 w-[60%] rounded-lg shadow-lg border border-gray-100">
              <ul>
              {
                suggestion?.map((suggestion)=><li className='py-2 px-3 shadow-sm hover:bg-gray-100' key={suggestion}> <FontAwesomeIcon className='w-4 h-4 mr-2 font-light search-icon cursor-pointer' icon={faMagnifyingGlass} />{suggestion}</li>)
              }
              </ul>
            </div>
